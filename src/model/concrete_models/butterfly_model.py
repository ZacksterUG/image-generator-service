...
class ButterflyModel(IModel):
    def __init__(self, model_path: Union[str, Path], device: Union[str, Path, torch.device], timestamps: int = 10):
        super().__init__()
    ...
    def create_model(self, model_path: Union[str, Path], device: torch.device) -> IModel:
        md = ButterflyModel(model_path, device)
        return md
